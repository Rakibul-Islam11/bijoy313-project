...
import { auth, authContext } from "../all-components/auth-porvider-context/AuthContext";
...
const VerificationPopup = ({
    ...
    const { popupOpen } = useContext(MobileMenuContext);
    const { user, loading, updateUserProfile, sendVerificationEmail } = useContext(authContext);
    ...
    useEffect(() => {
        if (!popupOpen || verificationMethod !== 'email' || !userProfile?.email) return;
    ...
    }, [popupOpen, verificationMethod, onClose, onVerificationSuccess, userProfile]);
    ...
    const verifyOtp = async () => {
        try {
            ...
            if (!userProfile?.phone || !userProfile?.email) return;
    ...
    const handleSendVerification = async () => {
        if (verificationMethod === 'email') {
            try {
                ...
                // Call the sendVerificationEmail function from auth context
            ...
            try {
                const success = await sendOtpToPhone();
                if (success) {
                    setVerificationSent(true);
                } else {
    ...
    if (!popupOpen || !userProfile) return null;
            ...
            <div className="bg-white p-6 rounded-lg max-w-md w-full shadow-xl transform transition-all">
                ...
                <div className="bg-blue-50 p-4 rounded-lg mb-6">
                    ...
                    <div className="flex items-center justify-between bg-white p-3 rounded border border-gray-200">
                        <p className="font-medium text-gray-800">
                            {verificationMethod === 'email' ? userProfile.email : userProfile.phone}
                        </p>
                        <button
                            onClick={() => {
                                navigator.clipboard.writeText(verificationMethod === 'email' ? userProfile.email : userProfile.phone);
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Copied!',
                ...
                {!verificationSent ? (
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Enter 6-digit OTP sent to {userProfile.phone}</label>
                            <input
                                type="text"
                                inputMode="numeric"
                                pattern="[0-9]*"
